...
def xnor_select(array, prc_1, prc_2) 
	selected = []
	array.each do |el|
		result_1 = prc_1.call(el)
		result_2 = prc_2.call(el)
		if result_1 && result_2 || !result_1 && !result_2
			selected << el  
		end
	end
...
def filter_out!(array, &prc)
	i = array.length - 1
	while i >= 0 
		array.delete_at(i) if prc.call(array[i])
		i -= 1
	end
...
def multi_map(array, num = 1, &prc)
	...
	num.times do
		(0...array.length).each do |i|
			new_array[i] = prc.call(new_array[i])
		end
	end
...
def selected_map!(array, p1, p2)
	array.each_with_index do |el, idx|
		if p1.call(el)
			array[idx] = p2.call(el)
		end
	end
...
def chain_map(num, prcs)
	...
	result = prcs[0].call(num)
	(1...prcs.length).each do |i|
		result = prcs[i].call(result)
	end
	result
end
