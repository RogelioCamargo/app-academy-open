...
def lrt_stops
  ...
  execute(<<-SQL)
		SELECT routes.stop_id, stops.name FROM routes
...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
		...
		FROM routes a 
		JOIN routes b ON (a.company = b.company AND a.num = b.num)
		JOIN stops stops_a ON stops_a.id = a.stop_id 
		JOIN stops stops_b ON stops_b.id = b.stop_id 
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
		SELECT DISTINCT 
			...
			to_destination.num, to_destination.company
		FROM 
			routes start 
		JOIN 
			routes to_transfer ON (
				start.company = to_transfer.company AND 
		...
		JOIN 
			routes from_transfer ON (
				transfer.id = from_transfer.stop_id
			)
		JOIN 
			routes to_destination ON (
				from_transfer.company = to_destination.company AND 
				from_transfer.num = to_destination.num
			)
		JOIN stops start_stops ON (
			start.stop_id = start_stops.id 
		)
		JOIN 
			stops destination_stops ON (
				to_destination.stop_id = destination_stops.id 
			)
		WHERE 
			start_stops.name = 'Craiglockhart' AND destination_stops.name = 'Sighthill'
  SQL
end